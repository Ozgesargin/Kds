...
# --- Sabitler ve Yollar
# İşlenmiş ASSISTments verisi
INPUT_FILE = os.path.join("../app/data", "output", "processed_skill_builder.csv")
# Model çıktı klasörü
...
CATEGORICAL_FEATURES = [
    # Gerekirse kategorik özellikleri buraya ekle
]
...
def create_mastery_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 2. Problem Zorluk Skoru (Statik, empirik p(correct))
...
def prepare_data(df: pd.DataFrame):
    """Veriyi train/test olarak böler, OHE ve ölçeklendirme yapar."""
    ...
    # Tüm sayısal sütunları float64 yap
    for col in NUMERICAL_FEATURES:
        if col in X.columns:
            X[col] = X[col].astype(np.float64)

    # Kategorik Kodlama (One-Hot Encoding, varsa)
    X = pd.get_dummies(X, columns=CATEGORICAL_FEATURES, dummy_na=False)

    # user_id bazında train/test bölme
    ...
    X_train.loc[:, numeric_cols_to_scale] = scaler.fit_transform(
        X_train.loc[:, numeric_cols_to_scale]
    )
    X_test.loc[:, numeric_cols_to_scale] = scaler.transform(
        X_test.loc[:, numeric_cols_to_scale]
    )
...
# ----------------------------------------------------------------------
def train_and_save(
    X_train, Y_train, X_test, Y_test, scaler: StandardScaler, feature_names: list[str]
):
    """Modeli eğitir, değerlendirir ve gerekli dosyaları kaydeder."""

    # Model Eğitimi (P(correct) tahmini)
    ...
    json.dump(feature_names, open(os.path.join(MODEL_OUTPUT_DIR, "feature_names.json"), "w", encoding="utf-8"))

    # NOT: difficulty_policy.json artık burada yazılmıyor.
    # Zorluk eşikleri ayrı bir "difficulty_calibration.py" scripti ile üretilecek.
...
if __name__ == "__main__":
    ...
    if not os.path.exists(INPUT_FILE):
        print(
            f"HATA: İşlenmiş veri bulunamadı: {INPUT_FILE}. "
            f"Lütfen data_preprocess.py dosyasını çalıştırın."
        )
        raise SystemExit(1)
